...
def simulate(X_train, X_test, errors, n, run_id):
    # Generate dataset
    features = pd.read_csv(f"simulations/layers_analysis/data/features.csv")[f"{run_id}"]
    ...
    models = [
        (harderLASSORegressor(hidden_dims=(20,), nu=0.1), 'harderLASSO ANN - (20,)'),
        (harderLASSORegressor(hidden_dims=(20, 10), nu=0.1), 'harderLASSO ANN - (20, 10)'),
        (harderLASSORegressor(hidden_dims=(20, 10, 5), nu=0.1), 'harderLASSO ANN - (20, 10, 5)'),
    ]
...
if __name__ == "__main__":
    ...
    X_train = pd.read_csv("simulations/layers_analysis/data/X_train.csv")
    X_test = pd.read_csv("simulations/layers_analysis/data/X_test.csv")
    errors = pd.read_csv("simulations/layers_analysis/data/errors.csv")


    for n in n_values:
        outdir = os.path.join(os.path.dirname(__file__), f'results/n{n}/Lasso_models/')
