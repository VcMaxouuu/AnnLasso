...
class AnnLassoRegressionModel(nn.Module):
    def __init__(
        ...
        # Pytorch Parameters
        self.activation = Custom_act_fun()
    ...
    def fit(self, print_epochs=True, graph=False):
        ...
        while self.Sd:
            ...
            test_loss = bare_loss_fn(predict(self, self.X_test), self.y_test) if hasattr(self, 'X_test') else ''
            ...
            if epoch % 100 == 0:
                ...
                if epoch != 0:
                    ...
                    if optimizer.param_groups[0]['lr'] == min_lr:
                        if loss < best_loss and epoch < 15000: #Gradient iterations limited here for limiting running time
                            best_loss = loss
                        else:
                            if print_epochs:
        ...
        curves_sd = {'epochs': epochs_history, 'train': train_loss_history, 'test': test_loss_history}
        
        #### FISTA part
        train_loss_history, test_loss_history, epochs_history = [],[],[]
        if self.n_Ista>0:
            lr, epoch, best_loss = min_lr, 0, float('inf')
            optimizer_penalized = FISTA(params=self.layer1.parameters(), lambda_=self.lamb, lr=lr)
            optimizer_unpenalized = FISTA(params=self.layer2.parameters(), lambda_=0.0, lr=lr)
            while True:
                y_pred = self(self.X)
                loss = loss_fn(y_pred, self.y, self.layer1)
                bare_loss = bare_loss_fn(y_pred, self.y)
                ...
                if epoch % 100 == 0:
                    ...
                    if epoch != 0:
                        if print_epochs:
                            if test_loss == '':
                                print(f"\tEpoch FISTA: {epoch} | Loss: {loss:.5f} | MSE on train set : {bare_loss:.5f} | w1 non zeros entries : {important_features(self)} | learning rate : {optimizer_penalized.param_groups[0]['lr']:.6f}")
                            else:
                                print(f"\tEpoch FISTA: {epoch} | Loss: {loss:.5f} | MSE on train set : {bare_loss:.5f} | MSE on test set : {test_loss:.5f} | w1 non zeros entries : {important_features(self)} | learning rate : {optimizer_penalized.param_groups[0]['lr']:.6f}")
                        if loss > best_loss:
        ...
        curves_ista = {'epochs': epochs_history, 'train': train_loss_history, 'test': test_loss_history}
        if graph:
            draw_curvers(curves_sd, curves_ista, self.lamb)


              
    def forward(self, X):
        layer1_output = self.activation(self.layer1(X))
        w2_normalized = F.normalize(self.layer2.weight, p=2, dim=1)
        logits = F.linear(layer1_output, w2_normalized, self.layer2.bias)
