...
import models.sub_models as models

class AnnLassoRegression(models.GeneralModel):
    ...
    def fit(self, X_train, y_train, print_epochs=True, set_name=None):
        ...
        if self.warm_start:
            ...
            for i in iterable:
                ...
                if i==-1:
                    model = models.RegressionAnn(X_train, y_train, lambi, self.lr, n_hidden_units=self.p2, n_ista=n_ista)
                elif i==5:
                    model = models.RegressionAnn(X_train, y_train, self.lambda_qut, self.lr, Sd=False, n_hidden_units=self.p2, n_ista=10000, init_weights=[model.layer1, model.layer2])
                else:
                    model = models.RegressionAnn(X_train, y_train, lambi, self.lr, n_hidden_units=self.p2, n_ista=n_ista, init_weights=[model.layer1, model.layer2])
            ...
            model = models.RegressionAnn(X_train, y_train, self.lambda_qut, self.lr, n_hidden_units=self.p2, n_ista=10000)
            curves_sd, curves_ista, layer1_history = model.fit(print_epochs)
